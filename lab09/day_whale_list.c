// Prints a days worth (LINKED LIST)
...
int check_dates (struct pod *first_pod, struct date *day);
...
void day_whales(struct pod *first_pod, struct date *day) {
    ...
    while (first_pod != NULL) {
        if (check_dates (first_pod, day) == 1){
...
int check_dates (struct pod *first_pod, struct date *day){
    ...
    if ((day->day == first_pod->when->day) && (day->month == first_pod
        ->when->month) && (day->year == first_pod->when->year)){
        result = 1;
    } 
