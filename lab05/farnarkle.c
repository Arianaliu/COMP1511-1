...
int count_farnarkles(int tiles1[N_TILES], int tiles2[N_TILES]) {
    ...
    while (i < N_TILES) {
        ...
        // If any of the tiles equal each other (same spot) farnarckle ++
        farN++;
        i++;
        } else {
        i++;
...
int count_arkles(int tiles1[N_TILES], int tiles2[N_TILES]) {
    ...
    // Records Elements in arrays to fix all elements in the end
    while (i< N_TILES){
        stored[i] = tiles1[i];
        storage[i] = tiles2[i];
        i++;
    ...
    if (farN < N_TILES){
        if (farN == 0){
                // No farnarkle arkle fixer
                while (i < N_TILES) {
                    if (tiles1[i] == tiles2[j] && i!=j){
                        tiles1[i] = -1;
                        tiles2[j] = -2;
                        arkles++;
                        // i++;
            ...
            // Farnarkle arkle counter
            while (i < N_TILES) {
                if (tiles1[i] == tiles2[j] && (i != store[i] && j!= 
                    store[j]) && i!=j){
                    tiles1[i] = -1;
                    tiles2[j] = -2;
                    arkles++;
    ...
    // Resets all arrays to normal values
    i = 0;
    while ( i < N_TILES){
        tiles1[i] = stored[i];
        tiles2[i] = storage[i];
...
void play_farnarkle(int hidden_tiles[N_TILES]) {
    ...
    int tiles1[N_TILES], tiles2[N_TILES];
    int i = 0, turn = 1;
    int farnarkles = 0, arkles = 0;
    
    while (i<N_TILES){ // To assign tiles1 some arbitrary values
        tiles1[i] = i;
        tiles2[i] = i;
        i++;
    }
    ...
    while (turn < 100){ // Max turns is 100
        printf("Enter guess for turn %d: ", turn);
        while (i < N_TILES){ // Scans input from user
            scanf("%d", &tiles2[i]);
            i++;
        }
        farnarkles = count_farnarkles(hidden_tiles, tiles2); // Gets farnarkles
        arkles     = count_arkles    (hidden_tiles, tiles2); // Gets arkles
        printf("%d farnarkles, %d arkles\n", farnarkles, arkles);
        if (farnarkles == 4){ // Win condition
            printf("You win\n");
            break;
        }
        i = 0;
...
void farnarkle_ai(int turn, int previous_guesses[MAX_TURNS][N_TILES],
    int farnarkles[MAX_TURNS], int arkles[MAX_TURNS], int guess[N_TILES]) {
