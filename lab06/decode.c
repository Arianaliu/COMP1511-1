...
int decrypt_caps (int character, int decap[], int cap[]);
int decrypt_notcaps (int character, int denotcap[], int notcap[]);
...
int main(int argc, char *argv[]) {
	...
	while (character != EOF){
		if (character >= 'A' && character <= 'Z'){
			character = decrypt_caps(character, decap, cap);
		} else if (character >= 'a' && character <= 'z'){
			character = decrypt_notcaps(character, denotcap, notcap);
		} else {
			putchar(character);
			character = getchar();
		}
